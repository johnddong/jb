...
var Member = (function() {
  ...
  ReceiverAddressElement =
    '<h5 class="text-label">收件人</h5>'+
    '<input type="text" name="receiver_name[]" class="input-info receiver-name" placeholder="收件人名稱" />'+
    '<h6 class="text-label">請填入收件人真實姓名，以確保順利收件</h6>'+
    '<h5 class="text-label">收件人連絡電話<span></span></h5>'+
    '<input type="text" name="receiver_mobile[]" class="input-info receiver-mobile" placeholder="收件人連絡電話" />'+
    '<h5 class="text-label">收件人地址<span></span></h5>'+
    '<div class="twzipcode">'+
      '<div class="address">'+
        '<div id="test" data-role="county" data-name="receiver_county[]" data-style="receiver-county"></div>'+
      ...
      $('<div />', {'class': c.addressBox})
        ...
        .append(
          '<form  name="shipping-info'+formCount+'" action="" method="post">'+
            '<div class="receiver-default-box"><label for="receiver-default'+formCount+'"><input type="radio" id="receiver-default'+formCount+'" name="receiver_default[]" class="receiver-default" value="1">&nbsp;設為預設</label></div>'+
            ReceiverAddressElement+
            '<div class="note">'+
              '<div class="pull-right">'+
                '<button type="submit" class="btn-submit submit"><span data-txt="新增">新增</span></button>'+
              '</div>'+
            '</div>'+
          '</form>'
        )
  ...
  createMiniReceiverAddress = function(obj){
      if(obj.default){
        var radio = '<input type="radio" name="receiver_default[]" class="receiver-default" value="1" checked>';
      ...
      '<div class="item">'+
        '<div class="mini-address-box table">'+
          '<div class="tr">'+
            '<div class="td">'+
              '<div class="receiver-default-box">'+ radio +
                '<label for="receiver-default01">'+
                  '&nbsp;預設'+
                '</label>'+
              '</div>'+
            '</div>'+
            ...
            '<div class="td btn-box">'+
              '<div class="btn-modify-box">'+
                '<a href="javascript:void(0);" class="btn-open-address">修改</a>'+
      ...
      $member.find('.saved-receiver-address').prepend(element);
  },
        ...
        $('<div />', {'class': 'address-box'})
          ...
          .append(
            '<form  name="shipping-info'+formCount+'" action="" method="post">'+
              ...
              '<div class="note">'+
                '<div class="pull-right">'+
                  '<button type="submit" class="btn-modify submit"><span data-txt="儲存變更">儲存變更</span></button>'+
                '</div>'+
              '</div>'+
            '</form>'
          );
  ...
  init = function() {
    ...
    $member.find('.btn-address').on('click', function() {
      ...
      $(this).parent().append(addressElement);
    ...
    $member.on('click','.'+c.btnOpenAddress,function(){
      ...
      if($miniAddressBox.hasClass('active')){
        $modifyAddressBox.append(addressElement).css('height','390px');
